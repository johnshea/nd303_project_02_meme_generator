...
class MemeEngine:
  ...
  def make_meme(self, img_path, text, author, width=500) -> str: #generated image path
      ...
      try:
          ...
          raise FileNotFoundError(f'File "{img_path}" was not found') from None
      except UnidentifiedImageError:
          raise UnidentifiedImageError(f"Unknown image type") from None

      out = im.resize(self._scale_image_dimensions(width, im.width, im.height))
  ...
  def _scale_image_dimensions(self, desired_width: int, image_width: int, image_height: int) -> tuple[int, int]:
  ...
  def _randomly_position_text(self, image: Image, draw: ImageDraw, quote: str) -> tuple[int, int]:
